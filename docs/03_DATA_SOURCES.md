...
- **Settlement lookup (spec drift, verified 2026-09-21):** two things about resolved markets differ from what the code was first written to expect.
  1. **The status filter and the status value use different vocabularies.** `GET /markets?status=settled` is the correct *filter* and returns resolved markets — but those markets come back with `"status": "finalized"`. Querying `?status=finalized` is rejected with HTTP 400 `invalid status filter`. Never filter on the value read off a market.
  2. **`GET /markets?tickers=a,b,c` is supported** and returns exactly those markets regardless of status, omitting tickers it doesn't know. This is what lets `settle` be driven from our own table (`02 §5`) instead of paging Kalshi's entire settled history. Chunk at 100 tickers per request to keep the URL short. Verified against the live API on 2026-09-21: 816/816 of our past-close tickers resolved in 9 requests, ~3s total.
...
- **Concurrency (2026-09-24):** each assessment takes ~22s, almost all of it waiting on news search and the LLM, and every sequential run in the ledger stopped on the wall clock at ~12 assessments — never on budget. `enrich` now keeps `enrich_concurrency` (config, default 4, max 8) assessments in flight via `modules/enrich/pool.ts`, lifting a 240s run to ~40. Both guards are checked only when an assessment *starts*, so in-flight work finishes: the time worst case is unchanged, and the daily cap can be overshot by at most `concurrency − 1` assessments (~$0.06 at 4).
