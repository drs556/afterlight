...
- **Every DB query bypasses Next's fetch cache** (`neon(url, { fetchOptions: { cache: "no-store" } })` in `src/db/index.ts`). neon-http queries are `fetch` calls, and Next 14 caches those in route handlers: without it, repeated identical queries returned stale rows and identical writes (ledger inserts, snapshots) silently never executed. Never create another Neon client without it. Smoke test: two back-to-back `/api/jobs/settle` calls must produce two `pipeline_runs` rows.
